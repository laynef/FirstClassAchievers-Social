...
export function getMessages(userId, otherId) {
	return function(dispatch) {
		...
		axios.get(`http://localhost:3214/api/messages/${userId}/${otherId}`)
			.then((response) => {
					dispatch({
...
export function createMessage(data) {
	return function(dispatch) {
		...
		axios.post(`http://localhost:3214/api/messages`, data)
			.then((response) => {
					dispatch({
...
export function inviteFriends(data, id, other) {
	return function(dispatch) {
		...
		axios.post(`http://localhost:3214/api/invite/${id}`, data)
			.then((response) => {
					dispatch({
