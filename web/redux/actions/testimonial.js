...
export function getTestimonials() {
	return function(dispatch) {
		...
		axios.get(`${process.env.LOCAL_PORT}/api/v1/testify`)
			.then((response) => {
				dispatch({
...
export function createTestimonials(data) {
	return function(dispatch) {
		...
		axios.post(`${process.env.LOCAL_PORT}/api/v1/testify`, data)
			.then((response) => {
				dispatch({
...
export function likeTestimonial(data, id) {
	return function(dispatch) {
		...
		axios.patch(`${process.env.LOCAL_PORT}/api/v1/like/testify/${id}`, data)
			.then((response) => {
				dispatch({
