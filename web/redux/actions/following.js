...
export function getFollowers(id) {
	return function(dispatch) {
		...
		axios.get(`https://still-wildwood-47727.herokuapp.com/api/v1/following/${id}`)
			.then((response) => {
				dispatch({
...
export function setFollowers(data, user, other) {
	return function(dispatch) {
		...
		axios.patch(`https://still-wildwood-47727.herokuapp.com/api/v1/following/${user}/${other}`, data)
			.then((response) => {
				dispatch({
