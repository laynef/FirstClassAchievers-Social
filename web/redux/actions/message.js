...
export function getMessages(userId, otherId) {
	return function(dispatch) {
		...
		axios.get(`${process.env.LOCAL_PORT}/api/v1/messages/${userId}/${otherId}`)
			.then((response) => {
				dispatch({
...
export function createMessage(data) {
	return function(dispatch) {
		...
		axios.post(`${process.env.LOCAL_PORT}/api/v1/messages`, data)
			.then((response) => {
				dispatch({
...
export function inviteFriends(data, id) {
	return function(dispatch) {
		...
		axios.post(`${process.env.LOCAL_PORT}/api/v1/invite/${id}`, data)
			.then((response) => {
				dispatch({
