...
export function getComment() {
	return function(dispatch) {
		...
		axios.get(`${process.env.PROXIMO_URL}/api/v1/comments`)
			.then((response) => {
				dispatch({
...
export function setComment(data, id) {
	return function(dispatch) {
		...
		axios.post(`${process.env.PROXIMO_URL}/api/v1/comments/${id}`, data)
			.then((response) => {
				dispatch({
...
export function likeComment(data, id) {
	return function(dispatch) {
		...
		axios.patch(`${process.env.PROXIMO_URL}/api/v1/like/comment/${id}`, data)
			.then((response) => {
				dispatch({
